...
El A* se ejecuta en background, repartido entre ticks:
  - Cada tick se le asigna un presupuesto de nodos (NODE_BUDGET).
  - Mientras A* no termina, BugNav mueve al bot.
...
PRESUPUESTO CPU:
────────────────
NODE_BUDGET = 30 nodos por tick. Con ~15-20µs por nodo en Python puro,
son ~450-600µs, dejando margen para el resto de la lógica del bot.
Ajustable si hay más/menos presupuesto disponible.
"""
...
NODE_BUDGET = 30          # nodos A* por tick
...
class AStarState:
    ...
    Para NODE_BUDGET=30, la lista crece despacio y la búsqueda lineal
...
# ---------------------------------------------------------------------------
# A* incremental: consume hasta `budget` nodos y luego para
# ---------------------------------------------------------------------------

def _astar_tick(state: AStarState, c: Controller,
                w: int, h: int, budget: int) -> None:
    if state.done:
        return
...
class BugNav:
    ...
    def moveTo(self, c: Controller, goal: Position, four_dirs: bool) -> Direction:
        ...
        if self._astar is not None and self._astar.is_active():
            _astar_tick(self._astar, c, w, h, NODE_BUDGET)
            if self._astar.done:
                if self._astar.path:
