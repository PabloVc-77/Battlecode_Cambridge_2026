...
class Ataque:
    def __init__(self, ct: Controller):
        ...
        # Contador de intentos fallidos de construcción de torreta
        self._build_retries: int = 0
        _BUILD_RETRY_LIMIT = 10  # turnos máximos intentando construir sin éxito
    ...
    def _work_objetivo(self, c: Controller):
        ...
        if bid is None or (bid is not None and c.get_team(bid) == c.get_team()
            ...
            # Si llevamos demasiados intentos sin construir, abandonar objetivo
            if self._build_retries >= 10:
                self._build_retries = 0
                self._block_objetivo(c)
                return
