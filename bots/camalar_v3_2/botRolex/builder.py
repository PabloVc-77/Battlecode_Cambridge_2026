...
class Harvester:
    def __init__(self, c: Controller):
        ...
        #   - self.layout_positions → full set of layout tile positions (avoid building there)
        self.layout_positions: set = set()
        self.end_bridges = []

        if self.spawn is not None:
            layout_info = compute_layout_for_core(c, self.spawn)
            self.end_bridges = layout_info['entry_positions']
            self.layout_positions = layout_info['layout_positions']
            for pos in self.end_bridges:
                c.draw_indicator_dot(pos, 245, 73, 39)

    def _in_bounds(self, pos: Position) -> bool:
        """Versión cacheada de _is_in_bounds — sin llamadas a la API."""
        return 0 <= pos.x < self.map_w and 0 <= pos.y < self.map_h

    def _is_layout_pos(self, pos: Position) -> bool:
        """True if pos is reserved for the Defensivo layout — don't build roads there."""
        return pos in self.layout_positions
    ...
    def _try_move(self, c: Controller, direction: Direction) -> bool:
        ...
        if (building_id is not None
            ...
            if c.can_destroy(dest):
                ...
                if c.can_build_road(dest) and not self._is_layout_pos(dest):
                    c.build_road(dest)
    ...
    def run(self, c: Controller):
        ...
        if target is not None:
            ...
            if c.can_build_harvester(target):
                ...
                if c.can_build_road(move_pos) and not self._is_layout_pos(move_pos):
    ...
    def place_bridge_ore(self, c: Controller):
        ...
        for d in places:
            ...
            if self._in_bounds(spot) and c.is_in_vision(spot) and not (spot in self.layout_positions and spot not in self.end_bridges):
    ...
    def revisar_camino_casa(self, c: Controller):
        ...
        if entity_type == EntityType.BRIDGE:
            ...
            # CONVEYOR / ARMOURED_CONVEYOR: la salida es check_pos + dirección
