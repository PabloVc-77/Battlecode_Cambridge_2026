...
BASE_LAYOUT = [
    # Priority 0 — starred: resource entry splitters (bridge targets for builder)
    ( 1, -2, EntityType.SPLITTER, "splitter", Direction.WEST,      0),  # <S*
    ( 2, -2, EntityType.SPLITTER, "splitter", Direction.SOUTH,     0),  # *Sv

    # Priority 1 — conveyor + splitter feeding down toward core
    ( 2, -1, EntityType.CONVEYOR, "conveyor", Direction.SOUTH,     1),  # CYv
    ( 2,  0, EntityType.SPLITTER, "splitter", Direction.SOUTH,     1),  # Sv

    # Priority 2 — left splitter of foundry chain
    (-1, -2, EntityType.SPLITTER, "splitter", Direction.WEST,      2),  # <S

    # Priority 3 — foundry & sentinels
    ( 0, -2, EntityType.FOUNDRY,  "foundry",  Direction.NORTH,     3),  # F
    (-1, -3, EntityType.SENTINEL, "sentinel", Direction.NORTHWEST, 3),  # T↖ upper-left
    ( 1, -3, EntityType.SENTINEL, "sentinel", Direction.NORTH,     3),  # T↑ upper-right
    ( 3, -2, EntityType.SENTINEL, "sentinel", Direction.NORTHEAST, 3),  # T↗ right-top
...
def compute_layout_for_core(c: Controller, core_pos: Position) -> dict:
    ...
    # Fallback: if no priority-0 entries usable, use priority-1 splitters closest to core
    if not entry_positions:
        fallback = []
        for dx, dy, etype, _, _, prio in layout:
            if etype != EntityType.SPLITTER or prio not in (1, 2):
                continue
            pos = Position(core_pos.x + dx, core_pos.y + dy)
            if not _is_in_bounds(c, pos):
                continue
            if c.is_in_vision(pos) and c.get_tile_env(pos) == Environment.WALL:
                continue
            fallback.append(pos)
        fallback.sort(key=lambda p: core_pos.distance_squared(p))
        entry_positions = fallback[:2]
